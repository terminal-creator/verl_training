#!/bin/bash
# ============================================
# GSPO (Group Self-Play Preference Optimization) 训练脚本
# ============================================
set -e
...
# 📂 数据配置
# ------------------------------
TRAIN_DATA="${SCRIPT_DIR}/data/example_gspo.parquet"  # 训练数据路径 (.json或.parquet)
VAL_DATA=""                                            # 验证数据路径 (可选)
...
MICRO_BATCH_SIZE=8                    # micro batch大小 (根据显存调整)
...
# 🔧 GSPO算法配置 (核心参数)
# ------------------------------
ROLLOUT_N=8                           # 每个prompt采样数 (GSPO核心参数，建议8-12)
ROLLOUT_TEMPERATURE=0.9               # 采样温度
# SELF_PLAY_ROUNDS=3                  # 自对弈轮数 (暂未使用，需自定义实现)

# ------------------------------
# 📊 偏好学习配置 (预留参数，当前verl版本暂未使用)
# ------------------------------
# PREFERENCE_BETA=0.1                 # 偏好优化温度 (暂未使用)
# USE_RANKING_LOSS=true               # 是否使用排序损失 (暂未使用)
# MARGIN=0.1                          # 排序边界 (暂未使用)
...
EXPERIMENT_NAME="gspo_$(date +%Y%m%d_%H%M%S)"          # 实验名称
...
WANDB_PROJECT="verl_gspo"             # WandB项目名称
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}   verl GSPO Training Script${NC}"
echo -e "${GREEN}   (Group Self-Play Preference Optimization)${NC}"
echo -e "${GREEN}========================================${NC}"

# 数据准备
echo -e "${YELLOW}[1/5] 准备数据...${NC}"
...
# 检查Reward Function
echo -e "${YELLOW}[2/5] 检查Reward Function...${NC}"
...
if command -v nvidia-smi &> /dev/null; then
    ...
    echo -e "${RED}错误: GSPO训练需要GPU${NC}"
    exit 1
fi

# 创建输出目录
echo -e "${YELLOW}[4/5] 准备输出目录...${NC}"
mkdir -p "$OUTPUT_DIR"/{logs,checkpoints}

# 打印配置摘要
echo -e "${YELLOW}[5/5] 启动GSPO训练...${NC}"
echo ""
echo "============================================"
echo "GSPO训练配置摘要:"
echo "============================================"
...
echo "每prompt采样:   $ROLLOUT_N"
echo "自对弈轮数:     $SELF_PLAY_ROUNDS"
echo "偏好Beta:       $PREFERENCE_BETA"
echo "GPU数量:        $N_GPUS"
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name=verl_gspo \
